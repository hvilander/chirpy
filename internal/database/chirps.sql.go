...
const getAllChripsByUserId = `-- name: GetAllChripsByUserId :many
SELECT id, created_at, updated_at, body, user_id from chirps where user_id = $1
`

func (q *Queries) GetAllChripsByUserId(ctx context.Context, userID uuid.NullUUID) ([]Chirp, error) {
	rows, err := q.db.QueryContext(ctx, getAllChripsByUserId, userID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
